	...
	void semAnaliz(LT::LexTable& lextable, IT::IdTable& idtable)
	{
		for (int i = 0; i < lextable.size; i++)
		{
				...
				if (i && lextable.table[i - 1].idxTI != 0xffff)
				{
					...
					for (int k = i + 1; lextable.table[k].lexema[0] != LEX_SEMICOLON; k++)
					{
						...
						if (lefttype == IT::IDDATATYPE::STR) // справа только литерал, ид или вызов строковой ф-ции
						{
							...
							if (l == LEX_PLUS || l == LEX_MINUS || l == LEX_STAR || l == LEX_PECENT || l == LEX_DIRSLASH) // выражени€ недопустимы
			...
			case LEX_ID: // проверка типа возвращаемого значени€  
			{
				...
				if (i && i >= 2 && lextable.table[i - 2].lexema[0] == LEX_FUNCTION)	// объ€вление функции
				{
					for (int k = i + 1; ; k++)
					{
						char l = lextable.table[k].lexema[0];
